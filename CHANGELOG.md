...
- Feat(llama.cpp, experimental): Add per-model `reasoning_control` option (under Dedicated Optimization → llama.cpp). When enabled (requires `apiMode: "openai"` and `optimization: "llama.cpp"`), the completion request carries `reasoning_control: true` and an "End Reasoning" button appears in the Copilot chat input status area while a completion is active; clicking it calls the server's `POST /chat/completions/control` endpoint (`action: "reasoning_end"`) to force-end the current reasoning block.
