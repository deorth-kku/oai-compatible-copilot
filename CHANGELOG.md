...
- Feat(llama.cpp, experimental): Add per-model `disk_kv_cache` option (under Dedicated Optimization → llama.cpp) to reuse the llama.cpp on-disk prompt (KV) cache across sessions. The extension tracks, per conversation, a cache id computed from model (base id) + reasoning effort + sanitized system prompt + tools, and decides restore/save per request: the first request of a new conversation restores `{cache_id}.bin` into an idle slot found via the server's `/slots` endpoint (pinned with `id_slot`) and — when the cache did not exist yet — saves it after the stream ends; resubmitting the first message with an unchanged combination does no slot work; changing the combination mid-conversation (reasoning level, model or tools) attempts a restore but never saves (the context is long at that point). Requires a llama.cpp server with `/slots` enabled (default; `--no-slots` disables it) and `--slot-save-path`. Known risk: swapping the model file behind the same id (or changing the KV quantization) may make a stale cache restore and serve wrong KV.
