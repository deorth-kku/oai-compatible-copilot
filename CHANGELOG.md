...
- Feat(llama.cpp, experimental): Server-side token counting. For models with `apiMode: "openai"` and `optimization: "llama.cpp"`, `provideTokenCount` now asks the llama.cpp server's `POST /chat/completions/input_tokens` endpoint for a REAL count (the server applies the loaded model's chat template and tokenizer) instead of using the local tiktoken estimator. The upstream cancellation token is wired straight into the request's fetch signal, so cancelling aborts the in-flight HTTP request. Any non-cancellation failure (e.g. 404 on llama.cpp builds without the endpoint, network error, missing API key) falls back to the local estimator.
