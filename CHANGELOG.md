...
- Fix: The reasoning replay cache is now keyed by a content hash of each assistant turn (its text + tool calls) instead of the turn's positional index in the message history. Context compression/summarization shortens the history and shifts indices, which previously made cached reasoning go stale (replay misses, and in the worst case one turn replaying another turn's reasoning). The entry is now written at the end of the streaming turn (the hash is only known once the turn's content has been emitted), and pre-existing positional cache entries are dropped on load since they cannot be mapped to content hashes.
- Feat(llama.cpp, experimental): Add per-model `split_system_prompt` option (under Dedicated Optimization → llama.cpp). When enabled (requires `apiMode: "openai"` and `optimization: "llama.cpp"`), the first system message is split at the first `</memoryInstructions>` marker: the stable prefix stays the system message so the llama.cpp prompt cache can hit it, while the variable tail (skills, agents, AGENTS.md attachments, template variables, the per-session `VSCODE_TARGET_SESSION_LOG` line) moves into a following user message (a second system message would be merged back together by llama.cpp's jinja template).
- Breaking: Remove the global `oaicopilot.stripTargetSessionLog` setting (and its config UI checkbox). The per-session `VSCODE_TARGET_SESSION_LOG` line is no longer stripped from the system prompt; use the new per-model `split_system_prompt` option instead.
...
- Feat(llama.cpp, experimental): Add per-model `disk_kv_cache` option (under Dedicated Optimization → llama.cpp) to reuse the llama.cpp on-disk prompt (KV) cache across sessions. The extension tracks, per conversation, a cache id computed from model (base id) + reasoning effort + sanitized system prompt + tools, and decides restore/save per request: the first request of a new conversation restores `{cache_id}.bin` into an idle slot found via the server's `/slots` endpoint (pinned with `id_slot`) and — when the restore missed — saves it after the stream ends; resubmitting the first message with an unchanged combination does no slot work; changing the combination mid-conversation (reasoning level, model or tools) attempts a restore but never saves (the context is long at that point). Requires a llama.cpp server with `/slots` enabled (default; `--no-slots` disables it) and `--slot-save-path`. Known risk: swapping the model file behind the same id (or changing the KV quantization) may make a stale cache restore and serve wrong KV.
- Feat(llama.cpp, experimental): The `disk_kv_cache` feature is now restart-aware. `GET /slots` is issued on every gated request, and when ALL slots are empty (no slot has ever been used since the server started — i.e. llama.cpp restarted and the in-memory KV is gone), a restore is attempted even in cases that would normally do no slot work (e.g. continuing a conversation after llama.cpp restarted mid-session, or after both the extension and llama.cpp restarted). A save now happens only on the first turn of a conversation after an attempted restore that missed (e.g. refilling a deleted `.bin`); not attempting a restore is no longer treated as a restore miss.
- Feat(llama.cpp, experimental): Add per-model `reasoning_control` option (under Dedicated Optimization → llama.cpp). When enabled (requires `apiMode: "openai"` and `optimization: "llama.cpp"`), the completion request carries `reasoning_control: true` and the token status bar becomes clickable while a completion is in flight (its tooltip gains a "Click To End Reasoning" hint): during token generation (TG) clicking it shows a picker of all registered in-flight streams, and selecting one calls the server's `POST /chat/completions/control` endpoint (`action: "reasoning_end"`) to force-end that stream's reasoning block; during prompt processing (PP) clicking it shows the same picker, which only lists streams already in TG. A stream is only registered once TG starts — its completion id arrives with the first PP-phase chunk, and a `reasoning_end` sent during PP would be rejected by the server. Once reasoning ends (the first delta carrying answer `content` or `tool_calls`), the stream is deregistered immediately rather than waiting for the stream to finish, so the picker only lists streams that are still reasoning.
- Fix: Remove the deprecated `managementCommand` property from the `languageModelChatProviders` contribution to clear the deprecation warning. The picker's "Manage" entry is no longer available for this vendor; use the `OAICopilot: Set OAI Compatible Apikey` command, the per-provider key command, or the configuration UI to set API keys (all existing flows are unchanged).
