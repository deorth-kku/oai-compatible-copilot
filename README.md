...
- `reasoning_control`: Enable real-time reasoning control for llama.cpp OpenAI-compatible requests. When enabled (requires `apiMode: "openai"` and `optimization: "llama.cpp"`), the token status bar becomes clickable while a completion is in flight and its tooltip shows a "Click To End Reasoning" hint: during token generation (TG) clicking it shows a picker of all registered in-flight streams and, on selection, calls the server's `POST /chat/completions/control` endpoint (`action: "reasoning_end"`) to force-end that stream's reasoning block; during prompt processing (PP) clicking it shows the same picker, which only lists streams already in TG (its own stream is not registered until TG starts). Defaults to false.
