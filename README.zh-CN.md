...
- `reasoning_control`：为 llama.cpp OpenAI 兼容请求启用实时推理控制。启用后（需要 `apiMode: "openai"` 且 `optimization: "llama.cpp"`），在补全进行中时 token 状态栏变为可点击（tooltip 显示 "Click To End Reasoning" 提示）：在 token 生成（TG）阶段点击会调用服务器的 `POST /chat/completions/control` 端点（`action: "reasoning_end"`）强制结束当前推理块；在提示词处理（PP）阶段点击会提示推理尚未开始。默认 false。
